...
**Room** (`stage.js`, `props.js`). The table props (lamp, firefly jar, letter blocks, ball, crayons, books, wooden star) are built from primitives and canvas textures, with no model files, and stay outside the open spread. Rendering goes through an `EffectComposer` with an `UnrealBloomPass` whose threshold (1.0) only catches things brighter than lit paper: bulbs, fireflies, glowing cut-outs and sparkles. A material meant to glow needs a colour above 1 (for example `MeshBasicMaterial` with `color.multiplyScalar(4)`).
